...
def create_stock_chart(symbol, date_from, date_to, chart_path):
    ...
    try:
        ...
        print("Error loading stock data CSV:", e)
        return False

    # Check if symbol exists in the dataframe
    if symbol not in df.columns:
        print(f"Symbol '{symbol}' not found in stock data. Available columns: {df.columns.tolist()}")
        return False
    ...
    try:
        ...
        print("After date conversion, first 5 rows:")
        print(df.head())
    except Exception as e:
        print(f"Error parsing dates with format DD/MM/YYYY: {e}")
        # Try alternative format as fallback
        try:
            df['Date'] = pd.to_datetime(df['Date'], dayfirst=True, errors='coerce')
            df = df.dropna(subset=['Date'])
            print("Used dayfirst=True parameter to parse dates")
        except Exception as e2:
            print(f"Alternative date parsing also failed: {e2}")
            return False

    # Parse input dates for filtering
    try:
        # Use dayfirst=True when parsing input dates to be consistent
        date_from_dt = pd.to_datetime(date_from, dayfirst=True)
        date_to_dt = pd.to_datetime(date_to, dayfirst=True)
        
        print(f"Filtering dates between {date_from_dt} and {date_to_dt}")
    except Exception as e:
        print(f"Error parsing input dates: {e}")
        return False

    if date_from_dt > date_to_dt:
        print("Invalid date range: 'date_from' is later than 'date_to'.")
        return False
    ...
    # Debug info
    print(f"DataFrame after filtering for dates between {date_from_dt} and {date_to_dt}:")
    print(f"Number of rows: {len(df_filtered)}")
    if len(df_filtered) > 0:
        print(f"Date range in filtered data: {df_filtered['Date'].min()} to {df_filtered['Date'].max()}")
        print(f"Sample data for {symbol}:")
        for idx, row in df_filtered.head().iterrows():
            print(f"  {row['Date'].strftime('%Y-%m-%d')}: {row[symbol]}")
    
    if df_filtered.empty:
        print(f"No stock data available for {symbol} between {date_from} and {date_to}.")
        return False
    ...
    if df_filtered[symbol].isna().any():
        print(f"Warning: {df_filtered[symbol].isna().sum()} missing values found for {symbol}. Interpolating...")
    ...
    try:
        ...
        if df_filtered[symbol].isna().any():
            print(f"Warning: Some price values couldn't be converted to numeric. Dropping those rows.")
            df_filtered = df_filtered.dropna(subset=[symbol])
    except Exception as e:
        print(f"Error converting prices to numeric: {e}")
        return False
    ...
    print(f"Final dataset has {len(dates)} data points")
    if len(dates) > 0:
        print("First few data points (date, price):")
        for i in range(min(5, len(dates))):
            print(f"  {dates[i].strftime('%Y-%m-%d')}: {prices[i]}")
    ...
    for date, price in zip(dates, prices):
        ...
        try:
            ...
            # For testing if function doesn't exist
            headlines = []
        except Exception as e:
            print(f"Error fetching news for {formatted_date}: {e}")
            headlines = []
    ...
    try:
        ...
        if os.path.exists(chart_path):
            ...
            print(f"Chart saved successfully at '{chart_path}', size: {file_size} bytes")
            return True
        else:
            print(f"ERROR: File was not created at '{chart_path}'")
            return False
    except Exception as e:
